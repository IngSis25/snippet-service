...
class RulesServiceCallServiceTest {
    @Mock
    ...
    @Test
    fun `formatSnippet should handle non-2xx response from formatter service`() {
        // Given
        ...
        ).thenReturn(ResponseEntity.status(HttpStatus.BAD_REQUEST).body(responseBody))
        ...
        try {
            ...
            assert(e.message?.contains("error") == true || e.message?.contains("400") == true)
        }
    }

    @Test
    fun `formatSnippet should handle empty response from formatter service`() {
        // Given
        val language = Language(id = 1L, name = "PrintScript", version = "1.0", extension = "ps")
        val snippetModel =
            Snippet(
    ...
    @Test
    fun `formatSnippet should handle response without formatted field`() {
        // Given
        val language = Language(id = 1L, name = "PrintScript", version = "1.0", extension = "ps")
        val snippetModel =
            Snippet(
        ...
        val snippet = FullSnippet(snippetModel, "print('hello')")
        val responseBody = mapOf("error" to "Something went wrong")
        ...
        try {
            ...
            assert(e.message?.contains("formatted") == true)
        }
    }

    @Test
    fun `lintSnippetSync should handle linter service non-2xx response`() {
        // Given
        val language = Language(id = 1L, name = "PrintScript", version = "1.0", extension = "ps")
        val snippetModel =
            Snippet(
        ...
        ).thenReturn(ResponseEntity.status(HttpStatus.BAD_REQUEST).body(emptyList<String>()))
